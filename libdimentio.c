...
kern_return_t
kread_buf_univ(kaddr_t addr, void *buf, mach_vm_size_t sz) {
    return kread_buf(addr, buf, sz);
}

kern_return_t
kwrite_buf_univ(kaddr_t addr, const void *buf, mach_msg_type_number_t sz) {
    return kwrite_buf(addr, buf, sz);
}
